...
class GenesisParser(RomInfoParser):
    ...
    * http://git.redump.net/mame/tree/src/mess/machine/md_slot.c
    ...
    def is_valid_data(self, data):
        ...
        if data[0x100 : 0x100 + 15] == b"SEGA MEGA DRIVE" or \
           data[0x100 : 0x100 + 12] == b"SEGA GENESIS":
            return True
        if self.has_smd_header(data) or self.is_interleaved(data):
            return True
        return False
    ...
    def parse_buffer(self, data):
        ...
        if self.has_smd_header(data):
            data = data[0x200 : ]
            self.deinterleave_smd(data)
        elif self.is_interleaved(data):
            self.deinterleave_md(data)
        ...
        props["publisher"] = self.get_publisher(props["copyright"])
    ...
    def deinterleave_smd(self, data):
        """
    ...
    def has_smd_header(self, data):
        ...
        return self.is_interleaved(data[0x200 : ])

    def deinterleave_md(self, data):
        """
    ...
    def get_publisher(self, copyright_str):
        """
